...
const ManageOrders = () => {
    ...
    useEffect(()=>{
        fetch('https://apartment-fis.herokuapp.com/all-orders')
        .then(res => res.json())
    ...
    return (
        <div>
            <div id="services" className="my-5 py-5" style={{background: '#F1F5F9', borderRadius: '100px'}}>
                <h1 className="text-center services__heading">Total <span className="underline-highlight ">{orders?.length}</span> Apartment Order Received</h1>

                <p className="text-center my-3" style={{"color": "gray"}}>You can cancel these orders at any time, but cant cancel Shipped orders</p>
                ...
                
                {
                    ...
                    {/* service name  */}
                            ...
                            {
                                order?.status.toString() === 'Shipped' ? <h6 className="text-center text-success">Apartment Shipped</h6>
                                :
                                <Button  
                                variant="danger" 
